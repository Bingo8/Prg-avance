...
obj1_t * obj1_creer() 
{
  ...
  if( ( obj1 = malloc(sizeof(obj1_t)) ) == NULL )
    {
      fprintf( stderr , "obj1_creer 1 : debordement memoire lors de la creation d'un objet de type obj1_t (%lu demandes)\n", 
  ...
  if( ( obj1->attributs = malloc( sizeof(char)*TAILLE ) ) == NULL )
    {
      fprintf( stderr , "obj1_creer 2 : debordement memoire lors de la creation des attributs d'un obj1_t (%lu demandes)\n", 
...
{
	obj1_t *tmp = malloc(sizeof(obj1_t));
	*tmp = **obj1;
	free(tmp->attributs);
	tmp->attributs = NULL;
	free(*obj1);
